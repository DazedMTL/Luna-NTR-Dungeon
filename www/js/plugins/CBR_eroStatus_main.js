...
* @param param1ValId
* @desc パラメータ１について
* @default 1_名前１,2_名前２,3_名前３,4_名前４,5_名前５,6_名前６
*
* @param param2ValId
* @desc パラメータ２について
* @default 11_名前１,12_名前２,13_名前３,14_名前４,15_名前５,16_名前６
*/
...
Game_System.prototype.initialize = function () {
  ...
  CBR_eroStatusState.param1ValId =
    param["param1ValId"].split(",").map((str) =>
      str.split("_").map((s, i) => {
        return i == 0 ? Number(s) : s;
      })
    ) || [];
  CBR_eroStatusState.param2ValId =
    param["param2ValId"].split(",").map((str) =>
      str.split("_").map((s, i) => {
        return i == 0 ? Number(s) : s;
      })
    ) || [];
  ...
  if (!this.CBR_complete()) {
    ...
    if (!flag) {
      ...
      for (var i = 0, len = ary.length; i < len; i++) {
        ...
        if (page == 0) {
          if (i == 0) {
            let arr = CBR_eroStatusState.param1ValId.sort((a, b) => {
              return $gameVariables.value(a[0]) > $gameVariables.value(b[0])
                ? -1
                : $gameVariables.value(a[0]) < $gameVariables.value(b[0])
                ? 1
                : 0;
            });
            temp = arr[0][1];
          } else if (i == 3) {
            let arr = CBR_eroStatusState.param2ValId.sort((a, b) => {
              return $gameVariables.value(a[0]) > $gameVariables.value(b[0])
                ? -1
                : $gameVariables.value(a[0]) < $gameVariables.value(b[0])
                ? 1
