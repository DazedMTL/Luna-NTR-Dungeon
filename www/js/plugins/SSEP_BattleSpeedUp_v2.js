...
Sepher.Param.BattleSpeedDefault = Number(
  Sepher.Parameters["BattleSpeed (Default)"]
);
Sepher.Param.BattleSpeedBoost = Number(
  Sepher.Parameters["BattleSpeed (Boost)"]
);
...
Sepher.Param.SwitchAnimePattern = Number(
  Sepher.Parameters["SwitchAnimePattern"]
);
...
if (Sepher.Param.VisibleSwitch) {
  ...
  Spriteset_Battle.prototype.createUpperLayer = function () {
    ...
    button.updateFrame = function () {
      ...
      if (QuickMode) {
        ...
        if (++duration == Sepher.Param.SwitchAnimeSpeed) {
          ...
          button.setFrame(
            animCount * Sepher.Param.SwitchWidth,
            Sepher.Param.SwitchTop * Sepher.Param.SwitchHeight,
            Sepher.Param.SwitchWidth,
            Sepher.Param.SwitchHeight
          );
          duration = 1;
        }
      }
    ...
    button.setFrame(
      Sepher.Param.SwitchLeft * Sepher.Param.SwitchWidth,
      Sepher.Param.SwitchTop * Sepher.Param.SwitchHeight,
      Sepher.Param.SwitchWidth,
      Sepher.Param.SwitchHeight
    );
...
if (Imported.YEP_VictoryAftermath) {
  ...
  BattleManager.isFinishedVictoryCheer = function () {
    if (Sepher.Param.YEPVictoryWait) {
      if (QuickMode || Input.isPressed("ok") || TouchInput.isPressed()) {
        return (
          ++this._victoryCheerWait >=
          Yanfly.Param.VACheerWait / Sepher.Param.BattleSpeedBoost
        );
      } else {
        return (
          ++this._victoryCheerWait >=
          Yanfly.Param.VACheerWait / Sepher.Param.BattleSpeedDefault
        );
      }
    } else {
