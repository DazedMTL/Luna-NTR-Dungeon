...
/*:
 * @plugindesc (v2.3) Apresenta um medidor de HP para os chefes.
 * @author Moghunter
 *
 * @param Shake Effect
 ...
 * HISTÓRICO
 * -----------------------------------------------------------------------------
 * v2.3 - Compatibilidade com MOG_ConsecutiveBattles.
...
Moghunter.bosshp_number_perMode = String(
  Moghunter.parameters["Number Percentage"] || "false"
);
...
Scene_Map.prototype.chronoCheckBossHPTerminate = function () {
  ...
  if (!battler.isDead()) {
    $gameSystem._chronoBossEnemy = battler;
    $gameSystem._battler_bhp_oldNumber[0] =
      this._spriteset._bosshp_sprites._hp_data[2];
    $gameSystem._battler_bhp_oldNumber[1] =
      this._spriteset._bosshp_sprites._hp_data[6];
...
// ** create Spriteset
//==============================
var _mog_bossHP_sbattle_createSpriteset =
...
// ** create Lower Layer
//==============================
var _mog_BossHP_sprMap_createLowerLayer =
...
Sprite_BossHP.prototype.initialize = function (init_battler) {
  ...
  this._hp_perMode =
    String(Moghunter.bosshp_number_perMode) === "true" ? true : false;
  this.createSprites();
...
Sprite_BossHP.prototype.onSkillCancelites = function (value) {
  if (this._init) {
    ...
    if (this._perSprite) {
      this._perSprite.opacity = this._layout.opacity;
    }
    for (var i = 0; i < 6; i++) {
      this._hp_number[i].opacity = this._layout.opacity;
    }
    return;
  }
