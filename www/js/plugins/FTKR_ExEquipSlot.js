...
(function () {
  ...
  FTKR.EES = {
    ...
    disabledWtpeIds: splitConvertNumber(
      parameters["Disabled Equip Same WtypeIds"] || ""
    ),
    disabledAtpeIds: splitConvertNumber(
      parameters["Disabled Equip Same AtypeIds"] || ""
    ),
    disabledSlotName: splitConvertNumber(
      parameters["Disabled Slot Name"] || ""
    ),
  };
  ...
  Window_EquipSlot.prototype.slotName = function (index) {
    return !FTKR.EES.disabledSlotName.contains(index + 1)
      ? _Window_EquipSlot_slotName.call(this, index)
      : "";
  };
  ...
  Window_EquipItem.prototype.checkEnabledMultiEquip = function (item) {
    return (
      !FTKR.EES.enable &&
      this._actor.isEquipped(item) &&
      !enableMultipleEquipment(item)
    );
  };
  ...
  Window_EquipItem.prototype.checkEquippedSameWtypeIds = function (item) {
    return (
      ...
      FTKR.EES.disabledWtpeIds.contains(item.wtypeId) &&
      this._actor.isEquippedSameWtypeIds(item.wtypeId)
    );
  };
  ...
  Window_EquipItem.prototype.checkEquippedSameAtypeIds = function (item) {
    return (
      ...
      FTKR.EES.disabledAtpeIds.contains(item.atypeId) &&
      this._actor.isEquippedSameAtypeIds(item.atypeId)
    );
  };
  ...
  Game_Actor.prototype.releaseUnequippableItems = function (forcing) {
    for (;;) {
      ...
      for (var i = 0; i < equips.length; i++) {
        ...
        if (
          item &&
          (!this.canEquip(item) || !matchEquipIds(item.etypeId, slots[i]))
        ) {
          if (!forcing) {
