...
BattleResult.prototype.updateExp = function () {
  //var targetMember = $gameParty.members().filter(actor => actor.isAlive())[0];
  var targetMember = $gameParty.members().filter(function (actor) {
    return actor.isAlive();
  });
  var firstMemberExp =
    $gameTemp._bResult[1] * (targetMember ? targetMember[0].exr : 1);
  //var firstMemberExp = $gameTemp._bResult[1] * ($gameParty.members()[0] ? $gameParty.members()[0].exr : 1) ;
  if (this.pressAnyKey() && firstMemberExp > 0) {
    this._expOld = firstMemberExp - 1;
  }
  var dif_number = this.update_dif(this._expOld, firstMemberExp, 15);
  ...
  if (this._expOld === firstMemberExp) {
    this.phaseAniClear();
    this._phase = 2;
