...
(function () {
  ...
  Game_Character.prototype.processMoveCommand = function (command) {
    if (command.code === Game_Character.ROUTE_PLAY_SE)
      playAdjustSe(command.parameters[0], this);
  ...
  AudioManager.updateAudioSource = function () {
    updateParameters(
      this._currentBgm,
      $gameMap.event($gameSystem._bgmSource),
      true
    );
    if ($gameSystem._bgsSources) {
      ...
      this.iterateAllBgs(
        function () {
          updateParameters(
            this._currentBgs,
            $gameMap.event($gameSystem._bgsSources[this.getBgsLineIndex()])
          );
        }.bind(this)
      );
    } else
      updateParameters(
        this._currentBgs,
        $gameMap.event($gameSystem._bgsSource)
      );
  };
  ...
  AudioManager.updateBgsParameters = function (bgs) {
    if ($gameMap && $gameSystem) {
      if ($gameSystem._bgsSources && this.getBgsLineIndex) {
        if ($gameMap.event($gameSystem._bgsSources[this.getBgsLineIndex()]))
          return;
      } else {
  ...
  var _AudioManager_bgmVolume = Object.getOwnPropertyDescriptor(
    AudioManager,
    "bgmVolume"
  );
  ...
  function updateParameters(audio, source, isBgm) {
    if (audio && source) {
      ...
      AudioManager.updateBufferParameters(
        buffer,
        AudioManager[isBgm ? "_bgmVolume" : "_bgsVolume"],
        audio
      );
  ...
  function adjust(audio, source) {
    if (!source)
      throw new Error("audiosourceエラー：音源となるイベントが存在しません");
    var listenerX,
      listenerY,
      listenerEvent = $gameMap.event($gameSystem._listenerEvent);
    if (listenerEvent) {
      ...
      switch (listener.toLowerCase()) {
        ...
        default:
          throw new Error(
            "audiosourceエラー：listenerパラメータはscreenかplayerにしてください"
          );
          break;
      }
    }
